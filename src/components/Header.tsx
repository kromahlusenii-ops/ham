...
export default function Header() {
  ...
  // Client-side auth check so this component works in static/ISR pages
  useEffect(() => {
    fetch("/api/auth/me")
      .then((r) => r.json())
      .then((data) => {
    ...
    >
      <nav className="mx-auto flex max-w-5xl items-center justify-between px-6 py-4 sm:px-8">
        ...
        <div className="hidden items-center gap-6 md:flex">
          ...
          <div className={`transition-opacity duration-200 ${authChecked ? "opacity-100" : "opacity-0"}`}>
            {isLoggedIn ? (
        ...
        {mobileOpen && (
          ...
          >
            <div className="flex flex-col gap-1 px-6 py-4">
              ...
              <div className="mt-2">
                {isLoggedIn ? (
                  <Button variant="primary" size="sm" href="/dashboard" onClick={() => setMobileOpen(false)}>
                    Dashboard
                  </Button>
                ) : (
                  <Button variant="primary" size="sm" href="/login" onClick={() => setMobileOpen(false)}>
                    Get Started
                  </Button>
                )}
              </div>
