...
/** Parse LLAMA.md / .llama/*.md files into normalized entries. */
export function parseLlamaFile(file: ContextFile): NormalizedEntry[] {
  ...
  for (const section of sections) {
    ...
    for (const bullet of bullets) {
      entries.push({
        id: stableHash(scope, "llama", bullet.text),
        scope,
        depth,
        source: "llama",
        authorMode: "human",
