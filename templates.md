...
- [Audit Log Template](#audit-log-template)
...
Tracks when memory audits were last run. The agent uses this to suggest periodic audits.
Auto-maintained: agent appends after each audit, keeps last 5 entries only.

| Date | Issues Found | Summary |
|------|-------------|---------|
| [setup date] | — | Initial setup |
```

The agent appends a row after each audit with the date, number of issues found, and a one-line summary. When the table exceeds 5 entries, remove the oldest row (keeping the header).
